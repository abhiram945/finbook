...
export const Navbar = () => {
    ...
    useEffect(() => {
        if (day && village) {
            const dayInUrl = days.find(d => d.dayName === day)
            setSelectedDay(dayInUrl);
            const villageInUrl = villages?.find(v => v.villageName === village)
    ...
    return <>
        <nav className="flex flexColumn">
            ...
            <div className="villagesContainer flex justifyLeft">
                {(loading && villages.length === 0) ? <Loader component="navbar" /> : villages.map((village, index) => <Link key={index} to={`${selectedDay.dayName}/${village.villageName}`}
                    onClick={() => { setSelectedVillage(village) }}
                    className={selectedVillage.villageName === village.villageName ? "active" : ""}>{village.villageName}</Link>)}
            </div>
